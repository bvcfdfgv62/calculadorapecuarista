...
const ProducerProfileSetup = lazy(() => import('./components/ProducerProfileSetup').then(m => ({ default: m.ProducerProfileSetup })))
...
function AppContent() {
  ...
  if (!user) {
    return (
      <AuthWrapper view="login" setView={() => {}}>
        <ProducerProfileSetup />
      </AuthWrapper>
    )
  }
    ...
    <div className="min-h-screen flex flex-col md:flex-row h-screen overflow-hidden" style={{ background: 'var(--background)' }}>
      ...
      <nav className="md:hidden fixed bottom-0 left-0 right-0 z-40 border-t shadow-[0_-4px_24px_rgba(0,0,0,0.12)]"
        ...
        <div className="flex items-center justify-around px-2 py-1.5">
          ...
          <MobileTabItem icon={<LogOut size={20} />} label="Sair" active={false} onClick={() => logout()} danger />
        </div>
      </nav>
    </div>
  )
